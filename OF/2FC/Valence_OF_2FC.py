...
from two_FC_layer_model_OF import Two_FC_layer
import os
import time
import gc
...
def train_func(train_loader, vfeature, vvalence, the_model, device, criter, optimizer, n_epochs, input_size, patience):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for (feature, valence) in train_loader:
            ...
            #-----------------------------------------------------------------------------
            # Cross Entropy Loss
            loss = criter(output.squeeze(1), valence.squeeze(1))  
            #-----------------------------------------------------------------------------
...
def loadingfiles(device):
    ...
    #
    h5file = h5py.File(os.path.join(dir_path, 'my_discrete_valence_OF.h5'), 'r')
    train_valence = {}
    for k, v in h5file.items():
...
def main(args):
    ...
    for index in range(0, movlistlength):
        ...
        # Save model
        # Model name
        model_name = movlist[index] + '_emobase2010_2FC_Valence_OF.pth'
        torch.save(model.state_dict(), os.path.join(args.model_path, model_name))

        #---------------------------------------------------------------------------------------------------------------
        # save predicted valence labels
        afilename = movlist[index] + '_predValence_emobase2010_2FC_OF.h5'
...
if __name__ == "__main__":
    #
    dir_path = '/home/minhdanh/Documents/2FC_OF'
