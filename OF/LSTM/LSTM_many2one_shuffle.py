...
class many2one_LSTM(torch.nn.Module):
    def __init__(self,OF_dim = 2048, reduced_dim=128, hidden_dim = 64, num_layers = 2, num_classes=7):  # hidden dim = 128
        super(many2one_LSTM, self).__init__()
        self.reduced_audio = nn.Linear(OF_dim, reduced_dim, bias=False)
        self.audio = OF_dim
