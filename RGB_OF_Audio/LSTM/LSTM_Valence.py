...
def train_dataloader_for_LSTM(trfeatures, trvalence, args):
    ...
    train_valence_for_chunks = train_valence[(args.seq_length - 1):len(train_valence)]
...
def validate_dataloader_for_LSTM(tfeatures, tvalence, args):
    ...
    tvalence_for_chunks = tvalence[(args.seq_length - 1):len(tvalence)]
...
def loadingfiles(device):
    ...
    #
    h5file = h5py.File(os.path.join(dir_path, 'my_discrete_valence_concat.h5'),
                       'r')
    train_valence = {}
    for k, v in h5file.items():
...
def main(args):
    ...
    for index in range(0, movlistlength):
        ...
        # Save model
        # Model name
        model_name = movlist[index] + '_2LSTM_Valence_RGB_OF_Audio.pth'
        torch.save(model.state_dict(), os.path.join(args.model_path, model_name))

        # ---------------------------------------------------------------------------------------------------------------
        # save predicted valence labels
        afilename = movlist[index] + '_predValence_2LSTM_classification_RGB_OF_Audio.h5'
...
if __name__ == "__main__":
    #
    dir_path = '/home/minhdanh/Documents/1LSTM_RGB_OF_Audio'
