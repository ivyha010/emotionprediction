...
from two_FC_layer_model_RGB_OF import Two_FC_layer
import os
import time
import gc
...
def loadingfiles(device):
    ...
    h5file = h5py.File(os.path.join(dir_path, 'rgb_OF_concat.h5'), 'r')
    train_features = {}
    ...
    #
    h5file = h5py.File(os.path.join(dir_path, 'my_discrete_valence_RGB_OF.h5'), 'r')
    train_valence = {}
    for k, v in h5file.items():
...
def main(args):
    ...
    for index in range(0, movlistlength):
        ...
        # Save model
        # Model name
        model_name = movlist[index] + '_emobase2010_2FC_Valence_RGB_OF.pth'
        torch.save(model.state_dict(), os.path.join(args.model_path, model_name))

        #---------------------------------------------------------------------------------------------------------------
        # save predicted valence labels
        afilename = movlist[index] + '_predValence_emobase2010_2FC_classification_RGB_OF.h5'
...
if __name__ == "__main__":
    #
    dir_path = '/home/minhdanh/Documents/2FC_RGB_OF'
