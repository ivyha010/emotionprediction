...
def loadingfiles(device):
    ...
    #
    h5file = h5py.File(os.path.join(dir_path, 'my_discrete_arousal_RGB_new.h5'), 'r')
    train_arousal = {}
    for k, v in h5file.items():
...
def main(args):
    ...
    for index in range(0, movlistlength):
        ...
        # Save model
        # Model name
        model_name = movlist[index] + '_emobase2010_2FC__Arousal_RGB.pth'
        torch.save(model.state_dict(), os.path.join(args.model_path, model_name))

        #---------------------------------------------------------------------------------------------------------------
        # save predicted arousal labels
        afilename = movlist[index] + '_predArousal_emobase2010_2FC_RGB.h5'
