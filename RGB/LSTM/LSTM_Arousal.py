...
def train_dataloader_for_LSTM(trfeatures, trarousal, args):
    ...
    train_arousal_for_chunks = train_arousal[0:(len(train_arousal)-(args.seq_length - 1))]
...
def validate_dataloader_for_LSTM(tfeatures, tarousal, args):
    ...
    tarousal_for_chunks = tarousal[0:(len(tarousal)-(args.seq_length - 1))]
...
def loadingfiles(device):
    ...
    h5file = h5py.File(os.path.join(dir_path,'RGB_features_ResNet50_standardized.h5'),'r')
    train_features = {}
    ...
    #
    h5file = h5py.File(os.path.join(dir_path, 'my_discrete_arousal_RGB.h5'),
                       'r')
    train_arousal = {}
    for k, v in h5file.items():
...
def main(args):
    ...
    for index in range(0, movlistlength):
        ...
        # Save model
        # Model name
        model_name = movlist[index] + '_2LSTM_Arousal_RGB.pth'
        torch.save(model.state_dict(), os.path.join(args.model_path, model_name))

        # ---------------------------------------------------------------------------------------------------------------
        # save predicted arousal labels
        afilename = movlist[index] + '_predArousal_2LSTM_classification_RGB.h5'
...
if __name__ == "__main__":
    #
    dir_path = '/home/minhdanh/Documents/LSTM_RGB'  # '/home/ubuntu/Documents/COGNIMUSE/'   # path to extracted features and arousal files
