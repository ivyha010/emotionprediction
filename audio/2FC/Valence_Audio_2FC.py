...
def loadingfiles(device):
    ...
    #
    h5file = h5py.File(os.path.join(dir_path, 'my_discrete_valence_Audio.h5'), 'r')
    train_valence = {}
    for k, v in h5file.items():
...
def main(args):
    ...
    for index in range(0, movlistlength):
        ...
        # Save model
        # Model name
        model_name = movlist[index] + '_emobase2010_2FC_Valence_Audio.pth'
        torch.save(model.state_dict(), os.path.join(args.model_path, model_name))

        #---------------------------------------------------------------------------------------------------------------
        # save predicted valence labels
        afilename = movlist[index] + '_predValence_emobase2010_2FC_Audio.h5'
